...
function processMessage(message: Message, client: Client, Discord: any) {
  ...
  if (message.mentions.users.has("626223136047628308")) {
    ...
    let getPrefix = async function (id: Snowflake, database: kv<any>) {
    ...
    if (message.guild !== null) {
      ...
      getPrefix(message.guild.id, db).then(prefix => {
        message.channel.send(
          "This is TigerDyno, a WIP bot developed by TigerGold59#8729. Use " +
            prefix +
            "info (on this server, global prefix is " +
            require("./config.json").global_prefix +
      ...
      message.channel.send(
        "This is TigerDyno, a WIP bot developed by TigerGold59#8729. Use " +
          require("./config.json").global_prefix +
          "info for a more complete description, and mention me on a server for my prefix.."
      );
    }
  }
}
