...
import { Guild, Snowflake, Message, Client } from "discord.js";
...
Please list the description and possible arguments in ./command_manuals.json

*/

interface RestrictionsBySnowflake {
  whitelist?: Snowflake[];
  blacklist?: Snowflake[];
}

interface CommandRestrictions {
  users?: RestrictionsBySnowflake;
  servers?: RestrictionsBySnowflake;
  channels?: RestrictionsBySnowflake;
}

interface Command {
  function: (message: Message, client: Client, Discord: any) => void;
  restrictions?: CommandRestrictions;
}

const getPrefix = async function (guild: Guild, database: kv<any>) {
  if (guild === null) {
...
function is_allowed(restrictions: CommandRestrictions, message: Message) {
  ...
  if (restrictions) {
    ...
    if (servers) {
      if (servers.blacklist && servers.whitelist) {
        log_console(
          chalk.magenta(
            "Invalid restrictions: servers has a whitelist and blacklist."
          )
        );
    ...
    if (is_allowed === true && channels) {
      if (channels.blacklist && channels.whitelist) {
        log_console(
          chalk.magenta(
            "Invalid restrictions: channels has a whitelist and blacklist."
          )
        );
    ...
    if (is_allowed === true && users) {
      if (users.blacklist && users.whitelist) {
        log_console(
          chalk.magenta(
            "Invalid restrictions: users has a whitelist and blacklist."
          )
        );
...
async function function_parser(message: Message, client: Client, Discord: any) {
  var commands: { [key: string]: Command } = {
    xofakind: {
      function: function (message: Message, client: Client, Discord: any) {
        ...
        if (num > 11 && !(message.author.id === "424564535030972426")) {
          message.channel.send(
            "Argument " +
              parts[1] +
              " is too high a number of dice, it would take hours for the bot to simulate this and millions of years to roll this yourself."
          );
          return;
        } else {
          if (num > 11 && message.author.id === "424564535030972426") {
            message.channel.send(
              "Warning: executive privilege has overriden the argument restriction. This command may take hours to process and hang the rest of the bot. Please restart the bot if this was not intended."
            );
          }
        }
        var tries = 0;
    ...
    progressbar: {
      function: function (message: Message, client: Client, Discord: any) {
        ...
        function createBar(percent: number, note: string) {
          var buffer = "[~";
          var lbuffer = "";
          var spacebuffer = "";
          ...
          buffer +=
            lbuffer + spacebuffer + "~] (" + percent + "%), note: " + note;
          return buffer;
        }
        var parts = message.content.split(" ");
        parts.shift();
        ...
        var note = parts[2];
        message.channel.send(createBar(startingpercent, note)).then(message => {
          let progressbars = require("./progressbars.json");
          Object.assign(progressbars, {
            name: (message as Message).id,
          });
          writeFile(
            "./progressbars.json",
    ...
    info: {
      function: function (message: Message, client: Client, Discord: any) {
        ...
        getPrefix(message.guild, db).then(prefix => {
          message.channel.send(
            "TigerDyno is an in-development project that will have many useful built-in commands and be easily customizable."
          );
          message.channel.send(
            "Use " + prefix + "commands for a list of commands and their uses."
          );
        });
      },
    },
    rolecount: {
      function: function (message: Message, client: Client, Discord: any) {
        ...
        message.channel.send(
          "Total roles for you: " + String(roles_array.length - 1)
        );
      },
    },
    ...
    commands: {
      function: function (message: Message, client: Client, Discord: any) {
        var cmd_manual_txt = require("./command_manual_uploader.js")(
          message.guild.id
        );
        const upload = require("./upload_to_pastebin.js");
        upload(cmd_manual_txt, (url: string) => {
          message.channel.send(
            "Command manual here: " + "https://paste.ee/r/" + url
          );
        });
      },
    },
    prefix: {
      function: function (message: Message, client: Client, Discord: any) {
        ...
        if (args.length === 1) {
          getPrefix(message.guild, db).then(prefix => {
            message.channel.send(
              "Prefix for " + message.guild.name + " is " + prefix + "."
            );
          });
        } else if (args.length === 2) {
          if (
            require("./config.json").admins.includes(
              String(message.author.id)
            ) === false
          ) {
            return message.channel.send(
              "You must be a bot admin to change the prefix here."
            );
          }
          if (
            ![
            ...
            getPrefix(message.guild, db).then(prefix => {
              message.channel.send(
                'Incorrect formatting. Correct usage is either "' +
                  prefix +
                  'prefix" for getting the prefix of this server, or "' +
                  prefix +
                  'prefix <char>" for setting the prefix of this server.'
              );
            });
          } else {
            db.set(String(message.guild.id), args[1]).then(() => {
              message.channel.send(
                "Set prefix for " + message.guild.name + " to " + args[1] + "."
              );
            });
          }
        } else {
  ...
  if (message.content.indexOf(prefix) === 0) {
    ...
    if (cmdobj) {
      if (cmdobj.function && is_allowed(cmdobj.restrictions, message)) {
        ...
        log_console(
          chalk.green(
            '"' + prefix + command + '" executed (' + message.id + ")"
          )
        );
        var is_module_cmd = true;
      } else if (
        cmdobj["function"] &&
        !is_allowed(cmdobj["restrictions"], message)
      ) {
        ...
        log_console(
          chalk.magenta(
            prefix +
              command +
              " was attempted and is restricted (" +
              message.id +
              ")"
          )
        );
  ...
  for (const module of modules) {
    ...
    let active_module_path = "./modules/" + module + "/main.js";
    ...
    if (
      module_obj["functions"][cmdname] &&
      is_allowed(module_obj["restrictions"], message)
    ) {
      log_console(
        chalk.green(
          "[Module " +
    ...
    if (is_module_cmd === false) {
      message.channel.send(
        "Sorry, that is not a recognized command. Please use " +
          prefix +
          "commands for a list of commands."
      );
      log_console(
        chalk.red(prefix + command + " is not a command (" + message.id + ")")
      );
    }
  }
}
